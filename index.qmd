...
ax.scatter(dreamy_data['patient'], dreamy_data['post_surgical_score'], 
           ...
           label='Doc Dreamy', edgecolors='#2E5A87', linewidths=0.5)
ax.scatter(duck_data['patient'], duck_data['post_surgical_score'], 
           marker='^', color='#E15759', s=60, alpha=0.7, 
           label='Doc Duck', edgecolors='#C13739', linewidths=0.5)

# Add horizontal dashed lines for means
ax.axhline(y=dreamy_mean, color='#4E79A7', linestyle='--', linewidth=1.5, alpha=0.8)
ax.axhline(y=duck_mean, color='#E15759', linestyle='--', linewidth=1.5, alpha=0.8)

# Add text annotations with shape symbols
# Position annotations at the right edge of the plot
x_max = patients_df['patient'].max()
x_pos = x_max * 0.98
ax.text(x_pos, dreamy_mean, f'○ Mean = {dreamy_mean:.2f}', 
        color='#4E79A7', fontsize=11, fontweight='bold',
        verticalalignment='center', horizontalalignment='right',
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', edgecolor='#4E79A7', alpha=0.8))
ax.text(x_pos, duck_mean, f'△ Mean = {duck_mean:.2f}', 
        color='#E15759', fontsize=11, fontweight='bold',
        verticalalignment='center', horizontalalignment='right',
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', edgecolor='#E15759', alpha=0.8))
...
# Calculate means within the stratification area (severity between -1 and 1)
# This shows the comparison within the overlap region where both doctors have patients
dreamy_stratified = dreamy_data[(dreamy_data['severity'] >= -1) & (dreamy_data['severity'] <= 1)]
duck_stratified = duck_data[(duck_data['severity'] >= -1) & (duck_data['severity'] <= 1)]
dreamy_mean = dreamy_stratified['post_surgical_score'].mean()
duck_mean = duck_stratified['post_surgical_score'].mean()

# Add shaded region highlighting severity between -1 and 1 (do this first so it's behind the points)
ax.axvspan(-1, 1, alpha=0.15, color='gray', zorder=0)

# Create a patch for the legend entry for the overlap region
overlap_patch = mpatches.Patch(facecolor='gray', alpha=0.15, edgecolor='gray', label='Overlap Region (-1 to 1)')

# Create scatter plots with different markers and colors
scatter_dreamy = ax.scatter(dreamy_data['severity'], dreamy_data['post_surgical_score'], 
                            marker='o', color='#4E79A7', s=60, alpha=0.7, 
                            label='Doc Dreamy', edgecolors='#2E5A87', linewidths=0.5, zorder=2)
scatter_duck = ax.scatter(duck_data['severity'], duck_data['post_surgical_score'], 
                          marker='^', color='#E15759', s=60, alpha=0.7, 
                          label='Doc Duck', edgecolors='#C13739', linewidths=0.5, zorder=2)

# Add horizontal dashed lines for means
ax.axhline(y=dreamy_mean, color='#4E79A7', linestyle='--', linewidth=1.5, alpha=0.8, zorder=1)
ax.axhline(y=duck_mean, color='#E15759', linestyle='--', linewidth=1.5, alpha=0.8, zorder=1)

# Add text annotations with shape symbols
# Position annotations at appropriate locations with overlap prevention
x_max = patients_df['severity'].max()
x_min = patients_df['severity'].min()
x_range = x_max - x_min
y_max = patients_df['post_surgical_score'].max()
y_min = patients_df['post_surgical_score'].min()
y_range = y_max - y_min

# Calculate distance between means relative to y-range
mean_distance = abs(dreamy_mean - duck_mean)
min_separation = y_range * 0.08  # Minimum separation threshold (8% of y-range)

# Set both annotations to the same x-position (in line with each other)
x_pos_dreamy = x_max - x_range * 0.05
x_pos_duck = x_max - x_range * 0.05

# Adjust vertical alignment based on which mean is higher to prevent overlap
if dreamy_mean > duck_mean:
    va_dreamy = 'bottom'
    va_duck = 'top'
else:
    va_dreamy = 'top'
...
ax.text(x_pos_dreamy, dreamy_mean, f'○ Mean (Overlap) = {dreamy_mean:.2f}', 
        color='#4E79A7', fontsize=10, fontweight='bold',
        verticalalignment=va_dreamy, horizontalalignment='right',
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', edgecolor='#4E79A7', alpha=0.8))
ax.text(x_pos_duck, duck_mean, f'△ Mean (Overlap) = {duck_mean:.2f}', 
        color='#E15759', fontsize=10, fontweight='bold',
        verticalalignment=va_duck, horizontalalignment='right',
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', edgecolor='#E15759', alpha=0.8))
...
# Create legend with all elements including the overlap region
ax.legend(handles=[overlap_patch, scatter_dreamy, scatter_duck], 
          labels=['Overlap Region (-1 to 1)', 'Doc Dreamy', 'Doc Duck'],
...
Taleb, Nassim Nicholas. 2017. “Surgeons Should Not Look Like Surgeons.” [https://medium.com/incerto/surgeons-should-notlook-like-surgeons-23b0e2cf6d52](https://medium.com/incerto/surgeons-should-notlook-like-surgeons-23b0e2cf6d52).
